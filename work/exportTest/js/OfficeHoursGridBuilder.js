...
function initArrays(data){
    undergradTAs = new Array();
    for(var i=0; i<data.undergrad_tas.length; i++){
        if(data.undergrad_tas[i].undergrad){
            undergradTAs.push(data.undergrad_tas[i]);
        }
    }
    ...
    for(var i=0; i<data.undergrad_tas.length; i++){
        if(!data.undergrad_tas[i].undergrad){
            gradTAs.push(data.undergrad_tas[i]);
        }
    }
}
...
function addUndergradTAs(data) {
    ...
    var tasPerRow = 4;
    var numTAs = undergradTAs.length;
    for (var i = 0; i < undergradTAs.length;) {
        var text = "";
        text = "<tr>";
        for (var j = 0; j < tasPerRow; j++) {
            text += buildTACell(i, numTAs, undergradTAs[i]);
            i++;
        }
        text += "</tr>";
        tas.append(text);
...
function addGraduateTAs(data) {
    var tas = $("#grad_tas");
    var tasPerRow = 4;
    var numTAs = gradTAs.length;
    for (var i = 0; i < gradTAs.length; ) {
        var text = "";
        text = "<tr>";
        for (var j = 0; j < tasPerRow; j++) {
            text += buildTACell(i, numTAs, gradTAs[i]);
            i++;
        }
        text += "</tr>";
        tas.append(text);
